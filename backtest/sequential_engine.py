...
def run_sequential_backtest(
        ...
        signals: np.ndarray, vol_arr: np.ndarray, params: GridParams, fee_bps: float, slippage_pct: float
) -> Tuple[List[float], Portfolio, GridPositionManager]:
    """
    Silnik sekwencyjny ODPORNY na look-ahead bias, wspierający dynamiczne bariery.
    """
    ...
    for t in range(1, len(close_p)):
        ...
        # Pobieramy zmienność obliczoną na podstawie danych do t-1
        current_vol = vol_arr[t - 1]

        # 1. Rozlicz pozycje z knotów (High/Low)
        closed_events = manager.update_market_price(c_high, c_low)
        for event in closed_events:
            portfolio.add_realized_trade(event["gross_pnl"], event["fee"])

        # 2. Przetwórz nowy sygnał z dynamiczną zmiennością
        manager.process_signal(c_close, prev_signal, current_vol)

        # 3. Zapisz kapitał (Mark-to-Market)
